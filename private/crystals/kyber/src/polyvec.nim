  ...
  when KYBER_POLYVECCOMPRESSEDBYTES == (KYBER_K * 352):
    ...
    for i in 0 ..< KYBER_K:
      for j in 0 ..< (KYBER_N div 8):
        ...
        t[2] = (a[aoff + 2].uint16 shr 6) or (a[aoff + 3].uint16 shl 2) or (a[
            aoff + 4].uint16 shl 10)
        t[3] = (a[aoff + 4].uint16 shr 1) or (a[aoff + 5].uint16 shl 7)
        t[4] = (a[aoff + 5].uint16 shr 4) or (a[aoff + 6].uint16 shl 4)
        t[5] = (a[aoff + 6].uint16 shr 7) or (a[aoff + 7].uint16 shl 1) or (a[
            aoff + 8].uint16 shl 9)
        ...
        for k in 0 ..< 8:
          r.vec[i].coeffs[8*j + k] =
            (((t[k] and 0x7FF'u16).uint32 * KYBER_Q.uint32 +
                1024'u32) shr 11).int16
      ...
      for j in 0 ..< (KYBER_N div 4):
        ...
        for k in 0 ..< 4:
          r.vec[i].coeffs[4*j + k] =
            (((t[k] and 0x3FF'u16).uint32 * KYBER_Q.uint32 +
                512'u32) shr 10).int16
