...
proc poly1305Process(h: var array[5, uint32], r, rr: openArray[uint32], msg: openArray[uint8], len: Natural) =
  if len == 0:
    return
...
proc poly1305Update(state: var Poly1305Ctx, input: openArray[uint8], len: Natural): int =
  if len == 0:
    return ErrNull
...
proc poly1305Digest(state: var Poly1305Ctx, digest: var openArray[uint8], len: Natural): int =
  if digest.len == 0:
    return ErrNull
