...
# NOTE: ported to Nim from https://github.com/Legrandin/pycryptodome/blob/master/src/poly1305.c
# NOTE: https://www.rfc-editor.org/rfc/rfc8439.txt
...
proc poly1305LoadR(r, rr: var array[4, uint32], secret: openArray[uint8]) =
  for i in 0 ..< 4:
...
proc poly1305LoadM(m: var array[5, uint32], data: openArray[uint8],
    len: Natural) =
  if len > 16:
...
proc poly1305Multiply(h: var array[5, uint32], r, rr: openArray[uint32]) =
  var
    a0, a1, a2, a3: uint64
  ...
  x0 = a0 * h[0].uint64 + aa0 * h[4].uint64 + aa1 * h[3].uint64 + aa2 * h[
      2].uint64 + aa3 * h[1].uint64
  x1 = a0 * h[1].uint64 + a1 * h[0].uint64 + aa1 * h[4].uint64 + aa2 * h[
      3].uint64 + aa3 * h[2].uint64
  x2 = a0 * h[2].uint64 + a1 * h[1].uint64 + a2 * h[0].uint64 + aa2 * h[
      4].uint64 + aa3 * h[3].uint64
  x3 = a0 * h[3].uint64 + a1 * h[2].uint64 + a2 * h[1].uint64 + a3 * h[
      0].uint64 + aa3 * h[4].uint64
...
proc poly1305Process(h: var array[5, uint32], r, rr: openArray[uint32],
    msg: openArray[uint8], len: Natural) =
  if len == 0:
    return
...
proc poly1305Init(state: var Poly1305Ctx, r, s: openArray[uint8]): int =
...
proc poly1305Update(state: var Poly1305Ctx, input: openArray[uint8],
    len: Natural): int =
  if len == 0:
    return ErrNull
...
proc poly1305Digest(state: var Poly1305Ctx, digest: var openArray[uint8],
    len: Natural): int =
  if digest.len == 0:
    return ErrNull
...
proc initWithRS*(state: var Poly1305Ctx, r, s: openArray[byte]) =
  discard poly1305Init(state, r, s)

proc newPoly1305Ctx*(key, nonce: openArray[byte], data: openArray[byte] = @[]): Poly1305Ctx =
  let (r, s, nonce) = derivePoly1305KeyPair(key, nonce)

  if poly1305Init(result, r, s) != Ok:
...
proc update*(state: var Poly1305Ctx, input: openArray[uint8]) =
  ## wrapper
  discard state.poly1305Update(input, input.len)


proc digest*(state: var Poly1305Ctx): array[16, uint8] =
...
proc hexDigest*(state: var Poly1305Ctx): string =
...
proc verify*(ctx: var Poly1305Ctx, macTag: openArray[uint8]): bool =
...
proc hexVerify*(ctx: var Poly1305Ctx, hexMacTag: string): bool =
