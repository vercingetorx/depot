...
proc initBlake2sCtx(ctx: var Blake2sCtx, key, salt, personal: openArray[byte],
    digestSize: int) =
  if not (digestSize <= maxDigestSize):
    raise newException(ValueError, "digest size exceeds maximum $1 bytes" %
        $maxDigestSize)
  ...
  if personal.len > 0:
    if not (personal.len <= maxPersonSize):
      raise newException(ValueError, "personalization size exceeds maximum $1 bytes" %
          $maxPersonSize)
