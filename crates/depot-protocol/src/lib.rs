...
use depot_core::{ErrorCode, PortablePermission, PortablePermissions, RemotePath};
...
pub fn decode_client_hello(payload: &[u8]) -> Result<ClientHello, CodecError> {
    ...
    Ok(ClientHello {
        version: payload[0],
        has_psk: payload[1] & (1 << 0) != 0,
        supports_client_auth: payload[1] & (1 << 1) != 0,
...
pub fn encode_server_hello(hello: &ServerHello) -> Vec<u8> {
    let mut flags = 0u8;
    if hello.require_psk {
        flags |= 1 << 0;
    }
    if hello.require_client_auth {
        flags |= 1 << 1;
    }
    if hello.sandboxed {
        flags |= 1 << 2;
...
pub fn decode_server_hello(payload: &[u8]) -> Result<ServerHello, CodecError> {
    ...
    Ok(ServerHello {
        version: payload[0],
        require_psk: payload[1] & (1 << 0) != 0,
        require_client_auth: payload[1] & (1 << 1) != 0,
        sandboxed: payload[1] & (1 << 2) != 0,
        features: decode_features(payload[2] as u64),
    })
}
...
pub fn encode_client_auth(payload: &ClientAuthPayload) -> Result<Vec<u8>, CodecError> {
    ...
    let mut encoded = Vec::with_capacity(payload.public_key.len() + payload.signature.len());
...
pub fn decode_client_auth(payload: &[u8]) -> Result<ClientAuthPayload, CodecError> {
    ...
    Ok(ClientAuthPayload {
        ...
        signature: payload[ML_DSA_87_PUBLIC_KEY_LEN..].to_vec(),
    })
}
