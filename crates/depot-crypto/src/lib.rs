...
use latebra::hash::Blake2b;
use latebra::kdf::HkdfHmacSha512;
use latebra::kem::{
...
impl Transcript {
    ...
    pub fn finish(&self) -> Result<[u8; 64], CryptoError> {
        let mut hasher = Blake2b::new_with_digest_length(64).map_err(CryptoError::Latebra)?;
        hasher.update(&self.bytes);
        to_array_64(hasher.finalize().as_bytes())
    }
...
impl HandshakeCryptoProvider for LatebraCrypto {
    ...
    fn derive_handshake_session_keys(
        ...
        let salt = transcript_salt(&transcript_hash, psk)?;
        let traffic_secret =
...
fn to_array_64(input: &[u8]) -> Result<[u8; 64], CryptoError> {
    input.try_into().map_err(|_| CryptoError::InvalidLength {
        expected: 64,
        actual: input.len(),
    })
...
mod tests {
    ...
    fn handshake_session_key_derivation_is_directional() {
        ...
        let client = crypto
            .derive_handshake_session_keys(shared_secret, transcript, None, true)
            .unwrap();
        let server = crypto
            .derive_handshake_session_keys(shared_secret, transcript, None, false)
            .unwrap();
