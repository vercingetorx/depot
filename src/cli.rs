...
use crate::config::{Config, ConfigError, config_dir, config_path};
use crate::core::{
...
impl ClientTrustProvider for ClientTrustStore {
    ...
    fn try_enroll(
        ...
        eprintln!(
            "[{}] {}",
            session_label,
            crate::ui::render_status("paired", &format!("trusted client {fingerprint}")),
        );
        Ok(true)
    }
}
...
async fn run_serve(app: &App, args: &ServeArgs) -> Result<(), RunError> {
    ...
    loop {
        ...
        eprintln!(
            "[{}] {}",
            session_id,
            crate::ui::render_status("connected", "client connected")
        );
            ...
            app.export_over_io_with_progress(
                ...
                |outcome| {
                    ...
                    match outcome.severity {
                        crate::core::OutcomeSeverity::ItemError
                        | crate::core::OutcomeSeverity::Fatal => {
                            let _ = console.print_error_line(&line);
                        }
                        crate::core::OutcomeSeverity::Success
                        | crate::core::OutcomeSeverity::Skipped => {
...
) -> Result<crate::app::ClientRuntimeOptions, RunError> {
    let config = load_config()?;
    let app = App::new(config);
