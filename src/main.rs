use clap::Parser;
use depot::cli::{Cli, RunError, run};
use depot::ui::{
...
fn render_run_error(error: &RunError) -> String {
    match error {
        ...
        RunError::App(depot::app::AppError::Depot(error)) => render_error(error, Audience::Client),
        RunError::App(depot::app::AppError::Handshake(error)) => {
            render_handshake_error(error, Audience::Client)
        }
        RunError::App(depot::app::AppError::Transport(error)) => {
